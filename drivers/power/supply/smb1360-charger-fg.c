...
static void smb1360_wakeup_src_init(struct smb1360_chip *chip)
{
	...
	chip->smb1360_ws.source = wakeup_source_register(chip->dev, "smb1360");
}
...
static int smb_parse_batt_id(struct smb1360_chip *chip)
{
	...
	if (rc >= 0) {
		chip->batt_id_chan = devm_iio_channel_get(chip->dev, "batt_id");
		if (IS_ERR(chip->batt_id_chan)) {
			rc = PTR_ERR(chip->batt_id_chan);
			if (rc != -EPROBE_DEFER)
				pr_err("batt_id channel unavailable, rc=%d\n", rc);
	...
	/* read battery ID */
	rc = iio_read_channel_processed(chip->batt_id_chan, &result);
	if (rc < 0) {
	 ...
	 * gives uA.
	 */
	batt_id_uv = (int64_t)result * 1000;
				...
				const struct i2c_device_id *id)
{
	...
	if (chip->debug_root) {
		...
		debugfs_create_x32("fg_access_type",
				   S_IFREG | 0644,
				   chip->debug_root,
				   &(chip->fg_access_type));
